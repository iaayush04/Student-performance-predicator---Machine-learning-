...
# ====================
df = pd.read_csv("data\\Synthetic_Student_Data.csv", sep=',')
...
def predict_single_student():
    ...
    try:
        ...
        if len(clf_model.classes_) == 2:
            pass_prob = clf_model.predict_proba(new_data)[0][1]
        else:
            pass_prob = 1.0 if clf_model.classes_[0] == 1 else 0.0
...
def predict_from_file():
    try:
        ...
        if missing:
            print(f"⚠️ Missing columns in input file: {missing}")
            return
        ...
        if len(clf_model.classes_) == 2:
            ...
            batch_df['Pass_Probability'] = 1.0 if clf_model.classes_[0] == 1 else 0.0
